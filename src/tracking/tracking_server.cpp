...
void TrackingServer::handle_client(int client_fd) {
    ...
    while (_running.load(std::memory_order_relaxed)) {
        ...
        while ((pos = pending.find('\n')) != std::string::npos) {
            ...
            // base::LogInfo() << "Tracking result frame:" << frame.frame_id
            //                 << " stream:" << frame.stream_id
            //                 << " object_count:" << frame.object_count;
            // for (const auto &object : frame.objects) {
            //     base::LogInfo() << "  object name:" << object.name << " position:[" << object.x
...
bool TrackingServer::parse_line(const std::string &line, TrackingFrame &frame) const {
    ...
    for (const Json::Value &object : objects) {
        ...
        tracking_object.width = position["w"].asInt();
        tracking_object.height = position["h"].asInt();
